 ...
 * images, then videos (a paired soundtrack's <Audio N> emitted just after
...
export const MAX = { picture: 90, video: 30, audio: 90, strings: 30, total: 210 };
// Skeleton sockets for a fresh Reference Splitter before a loader is wired.
// Reference Splitter: fixed output sockets (9 pictures / 3 videos /
// 3 paired soundtracks / 3 audios / 3 prompt strings) plus the static
// resolution/duration outputs.
const SPLIT_MEDIA_SOCKETS = [
  ...Array.from({ length: 9 }, (_, i) => ({ name: `picture_${i + 1}`, type: "IMAGE" })),
  ...Array.from({ length: 3 }, (_, i) => ({ name: `video_${i + 1}`, type: "IMAGE" })),
  ...Array.from({ length: 3 }, (_, i) => ({ name: `video_audio_${i + 1}`, type: "AUDIO" })),
  ...Array.from({ length: 3 }, (_, i) => ({ name: `audio_${i + 1}`, type: "AUDIO" })),
  ...Array.from({ length: 3 }, (_, i) => ({ name: `string_${i + 1}`, type: "STRING" })),
];
const SPLIT_RES_SOCKETS = [
  { name: "width_1", type: "INT" }, { name: "height_1", type: "INT" },
  { name: "width_2", type: "INT" }, { name: "height_2", type: "INT" },
...
const LANG_PAIRS = [
  ["Refresh output splitter", "刷新输出拆分器"],
  ["Clear media", "清空媒体"],
  ["Load files…", "加载文件…"],
  ...
  ["Soundtrack pairs with this video, labelled just after it", "音轨与此视频配对，标签排在其后面"],
  ["Soundtrack becomes a standalone audio reference — the video itself is not sent", "音轨作为独立音频参考发送 — 不发送视频画面"],
  ...
  ["Move the cursor to the start of the kept range", "将光标移到当前区间的开头"],
  ["Move the cursor to the end of the kept range", "将光标移到当前区间的结尾"],
  ["Move the cursor and kept range to the beginning of the track", "将光标和保留区间一起移到整段开头"],
  ["Move the cursor and kept range to the end of the track", "将光标和保留区间一起移到整段结尾"],
  ...
  ["← → step a frame (shift = 10 frames) · space play · [ ] set start/end here · C capture frame",
    "← → 逐帧（shift=10 帧）· 空格播放 · [ ] 在此设置起止 · C 截取帧"],
  ["← → nudge 0.1s (+shift = 1s) · space play · [ ] set start/end here",
    "← → 微调 0.1s（+shift=1秒）· 空格播放 · [ ] 在此设置起止"],
  ["The video's audio is ignored — nothing is extracted and no tag is created. Worth doing when the sound is irrelevant, since it also frees one of your reference slots.",
  ...
  ["Use alone to send only this video's soundtrack as a standalone audio reference — the video itself is not sent. Right for borrowing a speaker's voice timbre for a different character, referencing a music style, or lifting ambience without reusing the visuals.",
    "选择“独立”时只发送该视频的音轨作为独立音频参考 — 不发送视频画面。适合为不同角色借用说话人的音色、参考音乐风格或提取环境声，而不复用画面。"],
  ["The extracted track always gets its own AUDIO output — ComfyUI has no combined video-with-sound type, so the split is a wiring requirement. The mode decides which group it joins, which sets the native slot, the tag number, and whether the model binds it to that video's frames. Either way it occupies a reference slot, so a video with audio counts as two reference slots.",
    "拆出的音轨始终有独立的 AUDIO 输出 — ComfyUI 没有音视频合并类型，因此拆分是连线要求。模式决定它进入哪个分组，从而决定原生槽位、标签编号以及模型是否将其绑定到该视频的画面。无论哪种方式它都占用一个参考槽位，因此带音轨的视频会占用两个参考槽位。"],
];
...
const ZH_RULES = [
  ...
  [/^Load (\w+) — click to browse or drop a file \((\d+)\/(\d+)\)$/,
    (m, k, n, d) => `加载${tr(k)} — 点击浏览或拖入文件（${n}/${d}）`],
  [/^(\w+) (\d+)$/, (m, w, n) => `${tr(w)} ${n}`],
  ...
  [/^Open the prompt editor$/, "打开提示词编辑框"],
  [/^Optimize with Prompt Assistant$/, "使用提示词小助手优化"],
  [/^＋ Segment$/, "＋ 分段"],
  [/^Add a segment marker at the end time — click a numbered marker to select that segment$/,
    "在结束时间处添加分段标记 — 单击序号标记可选中该分段"],
  [/^↳ Use segment$/, "↳ 使用分段"],
  [/^Load the current segment as a new audio reference on the panel \(same file, trimmed to the segment\)$/,
    "将当前分段加载为新的音频素材（同一文件，仅选中分段时间）"],
  [/^All (\d+) audio slots are full — remove one before adding a segment\.$/,
    "所有 $1 个音频槽位已满 — 请先移除一个再添加分段。"],
  [/^Added segment ([\d.]+)–([\d.]+|end)s of (.+) as a new audio reference\.$/,
    (m, a, b, n) => `已将 ${n} 的分段 ${a}–${b}s 添加为新的音频参考。`],
  [/^－ Segment$/, "－ 分段"],
  [/^Delete the selected segment — click a numbered marker first$/,
    "删除当前选中的分段 — 请先单击一个序号标记"],
  [/^Whole clip, no crop, no segments$/, "整段音频，无裁剪、无分段"],
  [/^Segment (\d+): (.+) – (.+)$/, (m, n, a, b) => `分段 ${n}：${a} – ${b}`],
...
const EN_RULES = [
  ...
  [/^将当前分段加载为新的音频素材（同一文件，仅选中分段时间）$/,
    "Load the current segment as a new audio reference on the panel (same file, trimmed to the segment)"],
  [/^整段音频，无裁剪、无分段$/, "Whole clip, no crop, no segments"],
  [/^分段 (\d+)：(.+) – (.+)$/, (m, n, a, b) => `Segment ${n}: ${a} – ${b}`],
...
export function computeTags(all) {
  ...
  let p = 0, v = 0, va = 0, a = 0, s = 0;
  items.forEach((it) => { if (it.kind === "picture") tags.set(it, `<Picture ${++p}>`); });
  items.forEach((it) => {
    if (it.kind !== "video") return;
    // standalone mode sends only the soundtrack — the video gets no tag
    if (it.has_audio && it.audio_mode === "standalone") return;
    tags.set(it, `<Video ${++v}>`);
    if (it.has_audio && (it.audio_mode || "paired") === "paired")
      extra.set(it, `<V_Audio ${++va}>`);
  });
  // independent namespaces: video soundtracks (V_Audio) never offset the
  // standalone audio item numbering (Audio)
  items.forEach((it) => {
    if (it.kind === "video" && it.has_audio && it.audio_mode === "standalone")
      extra.set(it, `<V_Audio ${++va}>`);
    else if (it.kind === "audio")
      tags.set(it, `<Audio ${++a}>`);
    else if (it.kind === "string")
      tags.set(it, `<String ${++s}>`);
  });
  return { tags, extra };
}
...
export function fileCount(all) {
  ...
  (all || []).filter(isOn).forEach((it) => {
    if (it.kind === "string") return;   // text is free — not a file slot
    n += 1;
...
.mml-pes-modal{width:min(680px,92vw);height:min(560px,90vh);background:#141820;
  border:1px solid #2e3440;border-radius:10px;padding:10px;display:flex;
  flex-direction:column;gap:8px;box-shadow:0 12px 40px rgba(0,0,0,.5);}
.mml-pes-head{display:flex;align-items:center;gap:6px;}
.mml-pes-title{flex:1;font-size:12px;color:#d7dbe2;overflow:hidden;
  text-overflow:ellipsis;white-space:nowrap;}
.mml-pes-x{background:none;border:0;color:#8a93a3;cursor:pointer;font-size:13px;
  padding:2px 6px;}
.mml-pes-x:hover{color:#dfe4ec;}
.mml-pes-tawrap{position:relative;flex:1;display:flex;min-height:0;}
.mml-pes-ta{flex:1;width:100%;resize:none;background:#0e1116;color:#dfe4ec;
  border:1px solid #2e3440;border-radius:6px;padding:8px;
  font:12px ui-monospace,monospace;outline:none;overflow-y:auto;}
.mml-pes-ta:focus{border-color:#4a5568;}
.mml-pes-foot{display:flex;align-items:center;gap:8px;}
.mml-pes-count{font-size:10px;color:#6b7484;font-family:ui-monospace,monospace;}
.mml-pes-space{flex:1;}
.mml-pes-save,.mml-pes-cancel{border-radius:6px;cursor:pointer;
  font:11px system-ui,sans-serif;padding:4px 14px;background:#1b2230;
  border:1px solid #2e3440;color:#c9cfda;}
.mml-pes-save:hover,.mml-pes-cancel:hover{border-color:#4a5568;color:#dfe4ec;}
.mml-pes-save{border-color:#3a5a3a;color:#a8d8a8;}
...
.mml-slot.more{font-size:16px;color:#5c6472;}
.mml-slot.more:hover{color:#8a93a3;}
...
.mml-tmwaveWrap{position:relative;}
.mml-tmsegs{position:absolute;inset:0;pointer-events:none;}
.mml-tmseg{position:absolute;top:50%;transform:translate(-50%,-50%);
  pointer-events:auto;min-width:14px;height:14px;border-radius:7px;
  background:#2b313d;color:#9aa3b2;font:9px/14px ui-monospace,monospace;
  text-align:center;cursor:pointer;padding:0 3px;}
.mml-tmseg:hover,.mml-tmseg.cur{background:#4cc3e0;color:#0e1116;}
...
class TrimModal {
  constructor(panel, item) {
    ...
    // Audio segment boundaries; segments derive from them. Persisted on the
    // item as `segs` so markers survive Apply and reopening the editor.
    this.marks = Array.isArray(this.item.segs) ? [...this.item.segs] : [];
    // Logical cursor position: exact while paused (media.currentTime gets
    // quantised to frame boundaries, which made 0.1s nudges read as ~0.04s).
    this.cursorTime = this.start;
  ...
  key(e) {
    ...
    // Videos step by frame (shift = 10 frames); audio nudges by time.
    const step = this.item.kind === "video"
      ? (1 / (this.item.fps || TRIM_FPS)) * (e.shiftKey ? 10 : 1)
      : (e.shiftKey ? 1 : 0.1);
    const at = this.now();

    switch (e.key) {
      case "ArrowLeft":
        e.preventDefault(); this.seek(at - step); break;
      case "ArrowRight":
        e.preventDefault(); this.seek(at + step); break;
      case " ":
  ...
  apply() {
    ...
    if (this.marks.length) it.segs = [...this.marks];
    else delete it.segs;
      ...
      onclick: () => {
        if (this.media.paused) {
          if (this.media.currentTime < this.start ||
            ...
            this.media.currentTime = this.start;
          this.cursorTime = this.media.currentTime;
          this.media.play();
  ...
  seek(t, pause = true) {
    ...
    const tc = Math.min(Math.max(t, 0), this.dur);
    this.cursorTime = tc;
    try { this.media.currentTime = tc; } catch (e) {}
    // Show the seek target right away — media.currentTime can lag or be
    // quantised by the decoder, which makes 0.1s nudges look wrong.
    this.updatePlayhead(tc);
  }

  /** Displayed cursor: the seek target while paused, media time while
   *  playing (the decoder snaps seeks to frame boundaries). */
  now() {
    return this.media && !this.media.paused
      ? (this.media.currentTime || 0) : (this.cursorTime || 0);
  }
  ...
  buildTimeline() {
    ...
    if (this.item.kind === "audio") {
      ...
      this.segLayer = el("div", { class: "mml-tmsegs" });
      this.waveWrap = el("div", { class: "mml-tmwaveWrap" }, this.wave, this.segLayer);
    }
    ...
    this.layoutTimeline();
    const tl = el("div", { class: "mml-tmtimeline" },
      this.waveWrap || null, this.ruler, this.bar,
      el("div", { class: "mml-tmnow" },
        this.outside,
        el("span", { class: "mml-tmspace" }),
        el("span", {}, "cursor time"), this.playTime));
    if (this.item.kind === "audio") this.layoutSegments();
    return tl;
  }

  /** Kept-range presets: [start, end] spans between segment boundaries. */
  segments() {
    const b = [0, ...this.marks, this.dur];
    const segs = [];
    for (let i = 0; i < b.length - 1; i++) segs.push([b[i], b[i + 1]]);
    return segs;
  }

  /** Mark the end time as a segment boundary. */
  addSegment() {
    const t = this.end;
    const last = this.marks.length ? this.marks[this.marks.length - 1] : 0;
    if (t < last + 0.1 || t > this.dur - 0.1) return;
    this.marks.push(t);
    this.layoutSegments();
  }
  ...
  layoutSegments() {
    ...
    this.segLayer.replaceChildren(...this.segments().map((seg, i) =>
      el("span", { class: "mml-tmseg",
        style: { left: `${((seg[0] + seg[1]) / 2 / this.dur) * 100}%` },
        title: tr(`Segment ${i + 1}: ${fmt(seg[0])} \u2013 ${fmt(seg[1])}`),
        onclick: () => {
          this.start = seg[0];
          this.end = seg[1];
          this.seek(seg[0]);
  ...
  /** Add the current segment as a new audio item on the panel: the same
   *  full file, with trim pinned to the segment's time range. */
  useSegment() {
    const panel = this.panel;
    if (panel.count("audio") >= MAX.audio) {
      panel.say(`All ${MAX.audio} audio slots are full \u2014 remove one before ` +
        "adding a segment.", true);
      panel.render();
      return;
    }
    ...
    const eps = 0.05;
    const start = +this.start.toFixed(2);
    const end = this.end >= this.dur - eps ? null : +this.end.toFixed(2);
    panel.items.push({
      kind: "audio",
      file: this.item.file,
      name: `${this.item.name} \u00b7 ${start}-${end === null ? "end" : end}s`,
      duration: this.dur,
      width: null,
      height: null,
      has_audio: false,
      audio_mode: "off",
      trim: end === null ? { start } : { start, end },
    });
    panel.say(`Added segment ${start}\u2013${end === null ? "end" : end}s of ` +
      `${this.item.name} as a new audio reference.`);
    panel.commit();
  }

  /** Delete the segment whose range is currently selected (marker clicked). */
  removeSegment() {
    if (!this.marks.length) return;
    const i = this.segments().findIndex(([s, e]) =>
      Math.abs(s - this.start) < 0.05 && Math.abs(e - this.end) < 0.05);
    if (i < 0) return;
    // Merging the segment away drops its end boundary — the final segment
    // has none, so it drops its start boundary instead.
    if (i === this.marks.length) this.marks.pop();
  ...
  layoutTimeline() {
    ...
    this.checkOutside();
    if (this.segLayer) {
      [...this.segLayer.children].forEach((m, i) => {
        const [s, e] = this.segments()[i];
        m.classList.toggle("cur", Math.abs(s - this.start) < 0.05 &&
          Math.abs(e - this.end) < 0.05);
      });
    }
  ...
  updatePlayhead(t) {
    ...
    const at = t === undefined ? this.now() : t;
    // Clamp a little inside the bar: at exactly 0% or 100% the centred
    // marker is half outside and reads as missing.
    const pct = Math.min(Math.max((at / this.dur) * 100, 0.4), 99.6);
    this.playhead.style.left = `${pct}%`;
    // Two decimals so 0.1s nudges are visible in the readout.
    this.playTime.textContent = `${at.toFixed(2)}s`;
    this.checkOutside(at);
  }
  ...
  build() {
    ...
    const segBtn = isVid ? null : el("button", { class: "mml-btn mml-sm",
      title: "Add a segment marker at the end time — click a numbered " +
        "marker to select that segment",
      onclick: () => this.addSegment() }, "\uff0b Segment");
    const unsegBtn = isVid ? null : el("button", { class: "mml-btn mml-sm",
      title: "Delete the selected segment — click a numbered marker first",
      onclick: () => this.removeSegment() }, "\uff0d Segment");
    const useSegBtn = isVid ? null : el("button", { class: "mml-btn mml-sm",
      title: "Load the current segment as a new audio reference on the panel " +
        "(same file, trimmed to the segment)",
      onclick: () => this.useSegment() }, "\u21b3 Use segment");
      ...
      el("div", { class: "mml-tmmodal" + (isVid ? "" : " audio") },
        ...
        el("div", { class: "mml-tmfoot" },
          ...
          el("button", { class: "mml-btn mml-sm",
            title: "Move the cursor to the start of the kept range",
            onclick: () => this.seek(this.start) }, "\u21e4 start"),
          this.numStart, el("span", { class: "mml-tmdash" }, "\u2013"),
          this.numEnd,
          el("button", { class: "mml-btn mml-sm",
            title: "Move the cursor to the end of the kept range",
            onclick: () =>
              this.seek(Math.max(this.start, this.end - 0.05)) }, "end \u21e5"),
          this.readout,
          el("span", { class: "mml-tmspace" }),
          el("button", { class: "mml-btn mml-sm",
            title: "Move the cursor and kept range to the beginning of the track",
            onclick: () => { this.start = 0;
              this.seek(0); this.layoutTimeline(); } }, "\u23ee First"),
          el("button", { class: "mml-btn mml-sm",
            title: "Move the cursor and kept range to the end of the track",
            onclick: () => { this.end = this.dur;
              this.seek(this.dur - 0.05); this.layoutTimeline(); } },
            "Last \u23ed")),
        el("div", { class: "mml-tmfoot act" },
          segBtn, unsegBtn, useSegBtn,
          ...
          el("span", { class: "mml-tmspace" }),
          (this.item.trim || this.item.crop || this.marks.length)
            ? el("button", { class: "mml-btn mml-sm",
                title: "Whole clip, no crop, no segments",
                onclick: () => { this.start = 0; this.end = this.dur;
                  this.crop = null; this.cropMode = false;
                  this.marks = []; this.layoutSegments();
        ...
        el("div", { class: "mml-tmkeys" },
          isVid
            ? "\u2190 \u2192 step a frame (shift = 10 frames) \u00b7 space play \u00b7 " +
              "[ ] set start/end here \u00b7 C capture frame"
            : "\u2190 \u2192 nudge 0.1s (+shift = 1s) \u00b7 space play \u00b7 " +
              "[ ] set start/end here")));
    this.syncCrop();
...
class LoaderPanel {
  ...
  /** ✂ opens the prompt string editor popup: scrollable full text, with a
   *  bottom-right Prompt-Assistant icon when that extension is installed. */
  editPrompt(item) {
    // When the Prompt-Assistant extension is installed, the popup's modal is
    // marked as a dom-widget container so the assistant (sparkle optimize
    // icon + full button row) mounts into it exactly like on normal
    // textarea widgets.
    const assistantKey = (window.promptAssistant && window.FEATURES?.enabled)
      ? `mml_prompt_${Date.now()}` : null;
    let ta = null;

    const cleanupAssistant = () => {
      try {
        const instances = window.promptAssistant?.constructor?.instances;
        const inst = assistantKey && instances?.get(assistantKey);
        if (inst) { inst.container?.destroy?.(); instances.delete(assistantKey); }
        if (window.PromptAssistantInputWidgetMap && assistantKey)
          delete window.PromptAssistantInputWidgetMap[assistantKey];
        if (ta) {
          ta._promptAssistantMounted = false;
          delete ta._promptAssistantWidgetKey;
        }
      } catch (e) { /* best-effort cleanup */ }
    };
    const close = () => {
      cleanupAssistant();
      window.removeEventListener("keydown", this._pesEsc);
      overlay.remove();
    };

    ta = el("textarea", { class: "mml-pes-ta", spellcheck: "false" });
    ta.value = item.text || "";

    const count = el("span", { class: "mml-pes-count" }, `${ta.value.length} chars`);
    ta.addEventListener("input", () => { count.textContent = `${ta.value.length} chars`; });

    const save = () => { item.text = ta.value; this.commit(); close(); };
    const overlay = el("div", { class: "mml-pes-overlay",
      onmousedown: (e) => { if (e.target === overlay) close(); } },
      el("div", { class: "mml-pes-modal" },
        el("div", { class: "mml-pes-head" },
          el("span", { class: "mml-tag str" },
            (computeTags([item]).tags.get(item) || "").slice(1, -1)),
          el("span", { class: "mml-pes-title" }, item.name || "prompt"),
          el("button", { class: "mml-pes-x", title: "Close",
            onclick: () => close() }, "\u2715")),
        // the dom-widget marker anchors the assistant to the textarea's
        // bottom-right corner, above the Save/Cancel row
        el("div", { class: "mml-pes-tawrap dom-widget" }, ta),
        el("div", { class: "mml-pes-foot" },
          count,
          el("span", { class: "mml-pes-space" }),
          el("button", { class: "mml-pes-save", onclick: save }, "Save"),
          el("button", { class: "mml-pes-cancel", onclick: () => close() }, "Cancel"))));
    window.addEventListener("keydown", this._pesEsc = (e) => {
      if (e.key === "Escape") { e.stopPropagation(); close(); }
    });
    document.body.append(overlay);
    if (assistantKey) {
      try {
        window.promptAssistant.createAssistant(this.node, "prompt_text",
          { element: ta, inputEl: ta, name: "prompt_text" }, {}, assistantKey);
      } catch (e) { /* assistant integration is best-effort */ }
    }
    ta.focus();
  }
  ...
  /** The single trailing "+" slot: click to browse, drop to load. */
  moreSlot(kind) {
    const slot = el("div", { class: "mml-slot more",
      title: `Load ${kind} \u2014 click to browse or drop a file ` +
        `(${this.count(kind)}/${MAX[kind]})`,
  ...
  render() {
    ...
    if (pics.length < MAX.picture)
      picCells.push(this.moreSlot("picture"));
    left.append(el("div", { class: "mml-pics" }, picCells));

    // prompts: horizontal text bars, vertically aligned with the audio box
    const strings = this.items.filter((i) => i.kind === "string");
    left.append(el("div", { class: "mml-sec" }, "prompts",
      el("span", {}, `${strings.length}/${MAX.strings}`)));
    const strRows = strings.map((it) => {
      const input = el("input", { class: "mml-strinput", type: "text",
        placeholder: "prompt text\u2026", value: it.text || "" });
      const save = () => { it.text = input.value; this.commit(); };
    ...
    if (strings.length < MAX.strings)
      strRows.push(el("div", { class: "mml-strrow add",
        ...
        onclick: () => {
          this.items.push({ kind: "string", text: "", enabled: true });
          this.commit();
          const inputs = this.root.querySelectorAll(".mml-strinput");
          inputs[inputs.length - 1]?.focus();
        } }, "+"));
    left.append(el("div", { class: "mml-strs" }, strRows));
          ...
          el("span", { class: "mml-seg" },
            [["off", "关"], ["paired", "配对"], ["alone", "独立"]].map(([label, text]) => {
              ...
              return el("button", { class: m === mode ? "on" : "",
                title: m === "paired"
                  ? "Soundtrack pairs with this video, labelled just after it"
                  : m === "standalone"
                    ? "Send only the soundtrack as a standalone audio reference — the video itself is not sent"
    ...
    if (vids.length < MAX.video)
      vidCells.push(this.moreSlot("video"));
    ...
    if (auds.length < MAX.audio)
      audCells.push(this.moreSlot("audio"));
    ...
    vids.filter(isOn).forEach((i) => {
      if (i.has_audio && i.audio_mode === "standalone") return;  // shown below
    ...
    this.items.filter(isOn).forEach((i) => {
      if (i.kind === "video" && i.audio_mode === "standalone")
        order.push((extra.get(i) || "").slice(1, -1));
    });
    this.items.filter(isOn).forEach((i) => {
...
const SPLIT_HELP = [
  ...
  ["alone", "Send only this video's soundtrack as a standalone audio " +
    "reference \u2014 the video itself is not sent. Right for borrowing a " +
    "speaker's voice timbre for a different character, referencing a music " +
    "style, or lifting ambience without reusing the visuals."],
];

const SPLIT_WIRING = [
  ["paired", "video_audio_N", "ref_video_audio_0", "<Video 1> then <V_Audio 1>"],
  ["alone", "audio_N", "ref_audio_0", "video suppressed \u2014 only <V_Audio 1> is sent"],
];
  ...
  const box = el("div", { class: "mml-help" },
    ...
    el("div", { class: "mml-helpbody" },
      ...
      el("p", { class: "mml-helpnote" },
        ...
        "video's frames. Paired soundtracks get <V_Audio N> tags and count the " +
        "video as two reference slots; standalone audio gets <Audio M> and " +
        "sends only the soundtrack.")));
...
/** Restore a splitter's outputs to the full fixed 9/3/3/3/3 set plus the
 *  static resolution/duration sockets. Existing sockets are reused so
 *  their links survive; graph links are re-pointed afterwards. */
export function resetSplitterSockets(sp) {
  const byName = new Map(sp.outputs.map((o) => [o.name, o]));
  sp.outputs = [...SPLIT_MEDIA_SOCKETS, ...SPLIT_RES_SOCKETS].map((d) =>
    byName.get(d.name) || { name: d.name, type: d.type, links: null });

  // Socket indices moved — repoint stored links at their sockets.
  const slotOf = new Map();
  sp.outputs.forEach((o, i) => (o.links || []).forEach((id) => slotOf.set(id, i)));
  const links = sp.graph?.links;
  if (links) {
    for (const id in links) {
      const l = links[id];
      if (l && l.origin_id === sp.id && slotOf.has(l.id)) l.origin_slot = slotOf.get(l.id);
    }
  }
  try { sp.setDirtyCanvas?.(true, true); } catch (e) { /* Vue redraws */ }
}
 ...
 *  Builder connection. The splitter shows a fixed 9/3/3/3/3 socket set
 *  plus the resolution/duration outputs. */
export function addSplitter(node) {
  const existing = outputTargets(node, 0).find((n) => n.type === SPLITTER_NAME);
  if (existing) {
    safeCanvasFocus(existing);
    resetSplitterSockets(existing);
...
app.registerExtension({
  ...
  async beforeRegisterNodeDef(nodeType, nodeData) {
    ...
    nodeType.prototype.onNodeCreated = function () {
      ...
      const splitterBtn = this.addWidget("button", tr("Refresh output splitter"), null,
        () => addSplitter(this));
      if (splitterBtn) splitterBtn._mmxBase = "Refresh output splitter";
