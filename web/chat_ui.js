...
function injectStyles() {
  ...
  style.textContent = `
    ...
    .mmx-chat__apirow { display:flex; align-items:center; gap:6px; flex:0 0 auto; min-height:24px; }
    .mmx-chat__apilabel { color:#9ca3af; font-size:11px; }
    .mmx-chat__apiinput { flex:1 1 auto; min-width:0; height:24px; padding:0 8px; color:#cfd6de;
      background:#17181a; border:1px solid var(--border-color,#4b4d52); border-radius:4px;
      outline:none; font:12px ui-monospace,monospace; }
    .mmx-chat__apiinput:focus { border-color:#55a07e; }
    .mmx-chat__apitoggle { flex:0 0 auto; min-width:26px; height:24px; padding:0 6px; color:#9ca3af;
      background:#303d4b; border:1px solid #4b657d; border-radius:4px; cursor:pointer; font-size:12px; }
    .mmx-chat__apitoggle:hover { background:#3c5268; }
...
function setupChatNode(node) {
  ...
  // api_key lives in a hidden widget; the panel shows a masked password row
  const apiKeyWidget = node.widgets?.find((w) => w.name === "api_key");
  if (apiKeyWidget) hideWidget(apiKeyWidget);
  ...
  // masked API key row: dots by default, reveals only while editing
  const apiLabel = el("span", "mmx-chat__apilabel", "API Key");
  const apiKeyInput = el("input", "mmx-chat__apiinput");
  apiKeyInput.type = "password";
  apiKeyInput.placeholder = "sk-…";
  apiKeyInput.spellcheck = false;
  const apiKeyToggle = el("button", "mmx-chat__apitoggle", "👁");
  apiKeyToggle.type = "button";
  apiKeyToggle.title = "显示 / 隐藏 API Key";
  let apiKeyRevealed = false;
  const setApiKeyReveal = (reveal) => {
    apiKeyRevealed = reveal;
    apiKeyInput.type = reveal ? "text" : "password";
    apiKeyToggle.textContent = reveal ? "🙈" : "👁";
  };
  // keep input focus on mousedown so the blur/focus handlers don't fight
  // the click toggle (blur re-masks before the click can flip the state)
  apiKeyToggle.addEventListener("mousedown", (e) => e.preventDefault());
  apiKeyToggle.addEventListener("click", () => {
    setApiKeyReveal(!apiKeyRevealed);
    apiKeyInput.focus();
  });
  apiKeyInput.addEventListener("focus", () => setApiKeyReveal(true));
  apiKeyInput.addEventListener("blur", () => setApiKeyReveal(false));
  apiKeyInput.addEventListener("input", () => {
    if (apiKeyWidget) apiKeyWidget.value = apiKeyInput.value;
  });
  const syncApiKeyInput = () => {
    apiKeyInput.value = apiKeyWidget?.value || "";
    setApiKeyReveal(false);
  };
  syncApiKeyInput();
  const apiRow = el("div", "mmx-chat__apirow");
  apiRow.append(apiLabel, apiKeyInput, apiKeyToggle);
  main.append(flow, apiRow, messages, chips, composer, status);
  ...
  const findLinkedTextInput = () =>
    node.inputs?.find((i) => (i.name === "input_string" || i.name === "prompt" ||
      i.name === "user_prompt") && i.link != null) || null;
  ...
  const getLinkedPromptValue = () => {
    ...
    // Reference Splitter: resolve string_N through the loader's media_state
    // (the splitter's own widgets hold the ratio combos, not the strings).
    if (origin.type === "MiniMaxH3ReferenceSplitter") {
      const sockName = origin.outputs?.[link.origin_slot]?.name || "";
      const m = /^string_(\d+)$/.exec(sockName);
      const spLink = origin.inputs?.[0]?.link;
      const spLinkObj = spLink != null
        ? (typeof links?.get === "function" ? links.get(spLink) : links?.[spLink]) : null;
      const loader = spLinkObj ? node.graph?.getNodeById(spLinkObj.origin_id) : null;
      if (!loader || loader.type !== "MiniMaxH3MediaLoaderFantastic" || !m) return null;
      const index = parseInt(m[1], 10) - 1;
      const mediaState = loader.widgets?.find((w) => w.name === "media_state");
      try {
        const parsed = JSON.parse(mediaState?.value || "[]");
        const strs = (Array.isArray(parsed) ? parsed : [])
          .filter((i) => i.kind === "string" && i.enabled !== false);
        const item = strs[index];
        return item && item.text !== undefined ? String(item.text ?? "") : null;
      } catch (e) { return null; }
    }
  ...
  const refreshPromptLink = () => {
    const linked = findLinkedTextInput();
    promptLinked = !!linked;
    if (linked) {
      const value = getLinkedPromptValue();
      if (value !== null) {
        input.value = value;
        // the new string port loads into an editable dialog; the legacy
        // input_string/prompt links keep the read-only override behavior
        input.readOnly = linked.name !== "user_prompt";
        input.title = linked.name === "user_prompt"
          ? "已加载外部字符串（可直接编辑后发送）"
          : "已连接外部文本输入（发送时以外部文本为准）";
        status.textContent = input.title;
        status.dataset.state = "idle";
      }
    } else {
  ...
  domWidget.computeLayoutSize = () => ({
    ...
    minWidth: 480,   // matches the node's enforced minimum width
    maxHeight: 100000,
    maxWidth: 100000,
  });
