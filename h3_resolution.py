...
def _dual_ratio_options() -> list[io.DynamicCombo.Option]:
    """Ratio options that carry two independent size dropdowns each."""
    options = []
    for label in RATIO_LABELS:
        ratio = label.split(" ")[0]
        sizes = SIZE_OPTIONS_BY_RATIO[ratio]
        default = DEFAULT_SIZE_BY_RATIO[ratio]
        tooltip = f"Official reference-style {ratio} width×height presets (32-aligned)."
        options.append(
            io.DynamicCombo.Option(label, [
                io.Combo.Input("size1", options=sizes, default=default, tooltip=tooltip),
                io.Combo.Input("size2", options=sizes, default=default, tooltip=tooltip),
            ])
        )
    return options
...
class MiniMaxResolutionSelector(io.ComfyNode):
    """One aspect ratio driving two independent WxH preset dropdowns.

    Outputs both computed resolutions plus the requested video duration.
    """

    @classmethod
    def define_schema(cls):
        return io.Schema(
            node_id="MiniMaxResolutionSelector",
            display_name="MiniMax Resolution Selector",
            description=(
                "One aspect ratio window drives two size windows; outputs both "
                "width/height pairs and the requested video duration."
            ),
            category="model/conditioning/minimax",
            inputs=[
                io.DynamicCombo.Input(
            ...
            outputs=[
                io.Int.Output("width", tooltip="First size: width in pixels (multiple of 32)."),
                io.Int.Output("height", tooltip="First size: height in pixels (multiple of 32)."),
                io.Int.Output("width_2", tooltip="Second size: width in pixels (multiple of 32)."),
                io.Int.Output("height_2", tooltip="Second size: height in pixels (multiple of 32)."),
                io.Float.Output("duration", tooltip="Requested video duration (seconds)."),
            ],
        )

    @classmethod
    def execute(cls, aspect_ratio: dict, duration: float = 5.0) -> io.NodeOutput:
        if not isinstance(aspect_ratio, dict):
            raise ValueError(f"Invalid aspect_ratio payload: {aspect_ratio!r}")
        ratio_label = aspect_ratio.get("aspect_ratio") or ""
        ratio = ratio_label.split(" ")[0]
        if ratio not in RATIOS:
            raise ValueError(f"Unknown MiniMax H3 aspect ratio: {ratio_label!r}")
        width, height, _ = parse_size(aspect_ratio.get("size1"), ratio)
        width_2, height_2, _ = parse_size(aspect_ratio.get("size2"), ratio)
        return io.NodeOutput(width, height, width_2, height_2, float(duration))


class MiniMaxH3ResolutionExtension(ComfyExtension):
    async def get_node_list(self):
        return [MiniMaxH3ResolutionSelector, MiniMaxResolutionSelector]
