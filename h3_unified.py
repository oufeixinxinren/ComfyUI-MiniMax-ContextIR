...
MEDIA_TAG_RE = re.compile(
    r"<\s*(Image|Picture|Video|V_Audio|Audio)\s*(\d+)\s*>|"
    r"(?<![\w<])(Image|Picture|Video|V_Audio|Audio)\s*#?\s*(\d+)\b(?!\s*>)",
    re.IGNORECASE,
)
...
def prepare_prompt(prompt: str, counts: dict[str, int]) -> tuple[str, list[str]]:
    ...
    warnings: list[str] = []
    paired = int(counts.get("v_audio", 0))
    total_audio = int(counts.get("audios", 0))

    audio_tags = len(re.findall(r"<Audio\s+\d+>", normalized, re.IGNORECASE))

    def remap(match: re.Match) -> str:
        kind = match.group(1).lower()
        ordinal = int(match.group(2))
        if ordinal < 1 or ordinal > total_audio:
            warnings.append(
                f"<{match.group(1)} {match.group(2)}> is not connected; "
                f"available count is {total_audio}"
            )
            return match.group(0)
        if kind == "audio":
            # standalone audio items are encoded right after the paired
            # soundtracks in the model's reference order
            return f"<Audio {paired + ordinal}>"
        if ordinal > paired:
            # standalone video track: encoded after the standalone audio items
            return f"<Audio {audio_tags + ordinal}>"
        return f"<Audio {ordinal}>"   # paired soundtrack

    normalized = re.sub(r"<(V_Audio|Audio)\s+(\d+)>", remap, normalized, flags=re.IGNORECASE)
    limits = {
    ...
    if use_refs:
        ...
        for index, (video_ordinal, frames) in enumerate(ref_video_entries, 1):
            ...
            if soundtrack is not None:
                ...
                # soundtrack gets its own <Audio j> label, emitted after <Video k>
